...
const pagenateProps = defineProps<{
  current: number;
  max: number;
}>();
...
const post = computed(() => dataStore.getLocalDataList);

watch(
  () => pagenateProps.current,
  (newValue) => {
    // 초기화
    pageStart.value = 0;
    pageEnd.value = 8;
    pageStart.value = newValue === 1 ? 0 : newValue * 8 - 8;
    pageEnd.value =
      newValue !== pagenateProps.max || post.value.length % 8 === 0
        ? pageEnd.value * newValue
        : pageStart.value + (post.value.length % 8);
  }
);
</script>

<template>
  <div class="wrap row justify-start">
    <q-card
      v-for="(item, index) in post.slice(pageStart, pageEnd)"
      :key="index"
      class="my-card"
      @click.capture="path = item.id"
    >
